...
async def list_experiments(
    max_results: int = Query(default=100, ge=1, le=1000), service: MlflowService = Depends(get_mlflow_service)
) -> dict[str, Any]:
    return await service.list_experiments(max_results)
...
async def list_runs(
    experiment_ids: list[str] = Query(...), service: MlflowService = Depends(get_mlflow_service)
) -> dict[str, Any]:
    return await service.list_runs(experiment_ids)
...
async def list_registered_models(
    ...
    return await service.list_registered_models(max_results)
