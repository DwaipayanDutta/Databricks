...
from databricks_connector.schemas.jobs import WaitForRunRequest
from databricks_connector.services.jobs_service import JobsService
...
async def list_runs(
    ...
    offset: int = Query(default=0, ge=0, description="Deprecated by Databricks; prefer page_token."),
    ...
    return await service.list_runs(
        job_id=job_id, limit=limit, offset=offset, active_only=active_only, page_token=page_token
    )
...
async def wait_for_run(
    run_id: int,
    body: WaitForRunRequest = WaitForRunRequest(),
    service: JobsService = Depends(get_jobs_service),
) -> dict[str, Any]:
