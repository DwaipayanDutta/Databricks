...
from databricks_connector.services.unity_catalog_service import UnityCatalogService
...
def pagination_query(
    max_results: int | None = Query(
        ...
        ge=0,
        le=1000,
        description="Max results per page. Passing 0 opts into Unity Catalog's paginated response shape.",
    ),
    page_token: str | None = Query(
        default=None, description="Token from a previous response's next_page_token."
    ),
) -> tuple[int | None, str | None]:
    """Shared pagination query params for every Unity Catalog list endpoint."""
    return max_results, page_token


# --- Catalogs ---
@router.get(
    "/catalogs", summary="List catalogs", description="List catalogs, paginated via max_results/page_token."
)
async def list_catalogs(
    pagination: tuple[int | None, str | None] = Depends(pagination_query),
    service: UnityCatalogService = Depends(get_uc_service),
) -> dict[str, Any]:
    max_results, page_token = pagination
    return await service.list_catalogs(max_results=max_results, page_token=page_token)
...
@router.get(
    ...
    description="List schemas in a catalog, paginated via max_results/page_token.",
)
async def list_schemas(
    ...
    max_results, page_token = pagination
    return await service.list_schemas(catalog_name, max_results=max_results, page_token=page_token)
...
async def list_tables(
    ...
    return await service.list_tables(
        catalog_name, schema_name, max_results=max_results, page_token=page_token
    )
