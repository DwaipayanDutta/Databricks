...
from databricks_connector.schemas.clusters import (
    ClusterIdRequest,
...
from databricks_connector.services.cluster_service import ClusterService
...
@router.get(
    ...
    description="List clusters in the workspace, paginated via `page_token`/`next_page_token`.",
)
async def list_clusters(
    page_token: str | None = Query(
        default=None, description="Token from a previous response's next_page_token."
    ),
    limit: int | None = Query(default=None, ge=1, le=100, description="Max clusters to return per page."),
    service: ClusterService = Depends(get_cluster_service),
) -> dict[str, Any]:
    return await service.list_clusters(page_token=page_token, limit=limit)
...
@router.get(
    ...
    description="List lifecycle events for a cluster, paginated via `page_token`/`next_page_token`.",
)
async def get_cluster_events(
    cluster_id: str,
    page_size: int = Query(default=50, ge=1, le=100, description="Max events to return per page."),
    page_token: str | None = Query(
        default=None, description="Token from a previous response's next_page_token."
    ),
    start_time: int | None = Query(default=None, description="Start of the time range, epoch milliseconds."),
    end_time: int | None = Query(default=None, description="End of the time range, epoch milliseconds."),
    service: ClusterService = Depends(get_cluster_service),
) -> dict[str, Any]:
    return await service.get_cluster_events(
        cluster_id, page_size=page_size, page_token=page_token, start_time=start_time, end_time=end_time
    )
