...
class MlflowService:
    ...
    async def list_experiments(self, max_results: int = 100, page_token: str | None = None) -> dict[str, Any]:
        """Search/list experiments.

        Databricks' `experiments/search` endpoint requires POST with a JSON
        body (it does not accept query-string params); a GET request
        against it returns 405. `max_results`/`page_token` drive
        pagination.
        """
        body: dict[str, Any] = {"max_results": max_results}
        if page_token:
            body["page_token"] = page_token
        return await self._client.post(f"{_BASE}/experiments/search", json_body=body)
    ...
    async def list_runs(
        self, experiment_ids: list[str], max_results: int = 1000, page_token: str | None = None
    ) -> dict[str, Any]:
        body: dict[str, Any] = {"experiment_ids": experiment_ids, "max_results": max_results}
        if page_token:
            body["page_token"] = page_token
        return await self._client.post(f"{_BASE}/runs/search", json_body=body)
    ...
    async def list_registered_models(
        ...
        """List registered models via the (legacy) Workspace Model Registry.

        Note: Databricks recommends "Models in Unity Catalog"
        (`/api/2.1/unity-catalog/models`) for new integrations; the
        Workspace Model Registry used here is documented as being
        deprecated in the future but remains supported today.
        """
        params: dict[str, Any] = {"max_results": max_results}
        if page_token:
            params["page_token"] = page_token
        return await self._client.get(f"{_BASE}/registered-models/list", params=params)
