...
def _templated_path(request: Request) -> str:
    """Return the route's templated path (e.g. `/api/v1/jobs/{job_id}`)
    rather than the raw resolved path, to keep Prometheus label
    cardinality bounded regardless of how many distinct job/cluster/run
    IDs are requested.
    """
...
class MetricsMiddleware(BaseHTTPMiddleware):
    """Records Prometheus counters/histograms for every request served by
    the connector's own API (see `core/metrics.py` and `GET /metrics`).
    """
